...
@mcp.tool()
async def open_pinta() -> dict:
    """Open Pinta and wait for it to load"""
    global pinta_pid
    try:
        # Start Pinta
        process = subprocess.Popen(["pinta"], env=env)
        pinta_pid = process.pid
        
        # Allow time for it to open
        time.sleep(2)

        return {
            "content": [
                TextContent(type="text", text="Pinta opened successfully")
            ]
        }
    except Exception as e:
        return {
            "content": [
                TextContent(type="text", text=f"Error opening Pinta: {str(e)}")
            ]
        }
        
@mcp.tool()
async def draw_rectangle_in_pinta(x1: int, y1: int, x2: int, y2: int) -> dict:
    """Simulate drawing a rectangle in Pinta using the Paintbrush tool"""
    try:
        # Focus the Pinta window
        subprocess.run(["xdotool", "search", "--onlyvisible", "--name", "Pinta", "windowactivate"], env=env, check=True)
        time.sleep(0.5)

        # Select paintbrush tool (shortcut 'B')
        subprocess.run(["xdotool", "key", "b"], env=env, check=True)
        time.sleep(0.5)

        # Top edge
        subprocess.run(["xdotool", "mousemove", str(x1), str(y1)], env=env, check=True)
        subprocess.run(["xdotool", "mousedown", "1"], env=env, check=True)
        subprocess.run(["xdotool", "mousemove", str(x2), str(y1)], env=env, check=True)
        ...
        subprocess.run(["xdotool", "mousemove", str(x2), str(y2)], env=env, check=True)
        subprocess.run(["xdotool", "mousedown", "1"], env=env, check=True)
        subprocess.run(["xdotool", "mousemove", str(x1), str(y2)], env=env, check=True)
        subprocess.run(["xdotool", "mouseup", "1"], env=env, check=True)
        ...
        return {
            "content": [
                TextContent(type="text", text=f"Rectangle outlined with paintbrush from ({x1},{y1}) to ({x2},{y2})")
            ]
        }
        ...
        return {
            "content": [
                TextContent(type="text", text=f"Error drawing rectangle in Pinta: {str(e)}")
            ]
        }
        
@mcp.tool()
async def add_text_in_pinta(x: int, y: int, text: str) -> dict:
    """Simulate adding text to Pinta at given position"""
    try:
        # Focus Pinta window
        subprocess.run(["xdotool", "search", "--onlyvisible", "--name", "Pinta", "windowactivate"], env=env, check=True)
        time.sleep(0.5)
        ...
        subprocess.run(["xdotool", "key", "t"], env=env, check=True)
        time.sleep(0.5)

        # Move to position and click once to activate text area
        ...
        return {
            "content": [
                TextContent(type="text", text=f'Text "{text}" added at ({x},{y})')
            ]
        }
        ...
        return {
            "content": [
                TextContent(type="text", text=f"Error adding text in Pinta: {str(e)}")
            ]
        }
